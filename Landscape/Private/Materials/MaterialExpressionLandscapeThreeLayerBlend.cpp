...
UMaterialExpressionLandscapeThreeLayerBlend::UMaterialExpressionLandscapeThreeLayerBlend(const FObjectInitializer& ObjectInitializer)
...
void UMaterialExpressionLandscapeThreeLayerBlend::GetAllParameterInfo(TArray<FMaterialParameterInfo> &OutParameterInfo, TArray<FGuid> &OutParameterIds, const FMaterialParameterInfo& InBaseParameterInfo) const
{
...
UObject* UMaterialExpressionLandscapeThreeLayerBlend::GetReferencedTexture() const
{
...
void UMaterialExpressionLandscapeThreeLayerBlend::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.MemberProperty)
	{
		...
		if (PropertyName == GET_MEMBER_NAME_CHECKED(UMaterialExpressionLandscapeThreeLayerBlend, Layers))
		{
...
void UMaterialExpressionLandscapeThreeLayerBlend::GetCaption(TArray<FString>& OutCaptions) const
{
	OutCaptions.Add(FString(TEXT("3 Layer Blend")));
}

uint32 UMaterialExpressionLandscapeThreeLayerBlend::GetInputType(int32 InputIndex)
{
...
FName UMaterialExpressionLandscapeThreeLayerBlend::GetInputName(int32 InputIndex) const
{
...
const TArray<FExpressionInput*> UMaterialExpressionLandscapeThreeLayerBlend::GetInputs()
{
...
FExpressionInput* UMaterialExpressionLandscapeThreeLayerBlend::GetInput(int32 InputIndex)
{
	if (LayerWeightInputs.IsValidIndex(InputIndex))
	{
...
uint32 UMaterialExpressionLandscapeThreeLayerBlend::GetOutputType(int32 OutputIndex)
{
	switch (OutputIndex)
	{
	case 0:
...
int32 UMaterialExpressionLandscapeThreeLayerBlend::Compile(FMaterialCompiler* Compiler, int32 OutputIndex)
{
		...
		else
		{
			//Create Sort Function
			{
				...
				for (int32 Idx = 3; Idx < ActivedLayerWeights.Num(); Idx++)
				{
					...
					// // d >= maxr
					// FuncBody += TEXT("if( d.x >= maxr.x)\r\n { minr = medr; medr = maxr; maxr = d; }\r\n");
					// // d >= medr & d < maxr
					// FuncBody += TEXT("else if( d.x >= med.x)\r\n { minr = medr; medr = d;}\r\n");
					// // d >= minr & d < medr
					// FuncBody += TEXT("else if( d.x >= min.x)\r\n { minr = d;}\r\n");
